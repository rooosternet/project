class Profile < ActiveRecord::Base
	...
	scope :location_search, lambda {|search| joins(:user).where("LOWER(#{Profile.table_name}.location) LIKE LOWER(:p)",
                                               {:p => "%" + search.downcase + "%"})}

	scope :people_search, lambda {|search| joins(:user).where("LOWER(#{User.table_name}.firstname) LIKE LOWER(:p) OR
                                               LOWER(#{User.table_name}.lastname) LIKE LOWER(:p)",
                                               {:p => "%" + search.downcase + "%"})}
